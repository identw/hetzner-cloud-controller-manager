...
func (l *LoadBalancerOps) ReconcileHCLBTargets(
	...
	for _, node := range nodes {
		...
		if node.ObjectMeta.Labels[NameLabelType] == NameDedicatedNode {
			for _, i := range node.Status.Addresses {
				if i.Type == v1.NodeExternalIP {
					k8sDedicNodes[id] = i.Address
					k8sDedicNodesByIP[i.Address] = id
				}
			}
		} 
	}
	...
	for _, target := range lb.Targets {
		...
		if target.Type == hcloud.LoadBalancerTargetTypeIP {
			id = k8sDedicNodesByIP[target.IP.IP]
		}
		...
		if target.Type == hcloud.LoadBalancerTargetTypeIP {
			a, _, err = l.LBClient.RemoveIPTarget(ctx, lb, net.ParseIP(target.IP.IP))
		}
		if err != nil {
